  ...
  try {
    ...
    await sendMail({
      ...
      html: buildMailTemplate({
        ...
        buttonUrl: `${FRONTEND_URL}/orders/${order._id}`,
      }),
    });
...
router.post("/:id/support-message", auth, upload.single("image"), async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("ERREUR MAIL SAV :", mailErr.message);
    }
    ...
    if (sender === "admin") {
      try {
        ...
        console.log("ERREUR PUSH SAV :", pushErr.message);
      }
    }
    ...
    if (status === "Expédiée") {
      await sendMail({
        ...
        html: buildMailTemplate({
          ...
          buttonUrl: trackingUrl || `${FRONTEND_URL}/orders/${order._id}`,
        }),
      });
